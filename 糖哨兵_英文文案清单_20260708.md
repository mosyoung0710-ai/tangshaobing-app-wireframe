# 糖哨兵 App 英文文案清单

- 品牌英文名：GluSentry
- 用途：供多语言文案评审、二次润色和前端实现参考。
- 说明：表格按页面/流程归并；同一页面的输入态、异常态、明文/暗文态已去重。

## 通用与导航（全局通用）

| 中文 | 英文建议 |
|---|---|
| 糖哨兵 | GluSentry |
| 您的全天候血糖守护者 | Your 24/7 glucose guardian |
| 设备 | Device |
| 我的 | Me |
| 设置 | Settings |
| 下一步 | Continue |
| 保存 | Save |
| 完成 | Done |
| 确定 | Confirm |
| 取消 | Cancel |
| 重试 | Retry |
| 返回 | Back |
| 加载中 | Loading... |
| 设置中 | Setting up... |
| 保存中 | Saving... |
| 请稍候 | Please wait. |
| 暂无数据 | No data |
| 已设置 | Set |
| 未设置 | Not set |
| 已连接 | Connected |
| 未连接 | Not connected |
| 已应用 | Applied |
| 应用 | Apply |
## 页面：登录 / 注册 / 忘记密码（对应设计图 1-13）

| 中文 | 英文建议 |
|---|---|
| 验证码登录 | Sign in with Code |
| 密码登录 | Sign in with Password |
| 请输入手机号码 | Mobile number |
| 请输入账号密码 | Password |
| 请输入验证码 | Verification code |
| 获取验证码 | Get Code |
| 52s 后重发 | Resend in 52s |
| 登录 | Sign In |
| 忘记密码？ | Forgot password? |
| 没有账号？ | No account? |
| 立即注册 | Register Now |
| 已有账号？ | Already have an account? |
| 去登录 | Sign In |
| 未注册的手机号验证通过后将自动注册并登录 | Unregistered mobile numbers will be registered and signed in automatically after verification. |
| 我已阅读并同意《糖哨兵服务条款》和《隐私政策》 | I have read and agree to the GluSentry Terms of Service and Privacy Policy. |
| 《糖哨兵服务条款》 | GluSentry Terms of Service |
| 《隐私政策》 | Privacy Policy |
| 同意并继续 | Agree and Continue |
| 请输入正确手机号码 | Use a valid mobile number. |
| 请输入正确密码 | Check your password. |
| 验证码已超时/验证码错误/验证码失效 | Verification code expired / incorrect / invalid. |
| 欢迎注册糖哨兵 | Welcome to GluSentry |
| 请输入 6–16 位密码，支持数字、字母及符号 | Use 6–16 characters, including numbers, letters, or symbols. |
| 忘记密码 | Forgot Password |
| 请输入手机号码和验证码 | Use your mobile number and verification code. |
| 设置新密码 | Set New Password |
| 输入新密码 | New Password |
| 您的新密码必须不同于以前使用的密码 | Your new password must be different from your previous password. |
| 请输入新密码 | New password |
| 请再次输入密码 | Confirm password |
| 密码长度不能少于 6 位 | Password must be at least 6 characters. |
| 两次密码不一致，请检查 | Passwords do not match. Please check. |

## 页面：添加设备 / 搜索设备 / 蓝牙连接（对应设计图 14-21）

| 中文 | 英文建议 |
|---|---|
| 添加设备 | Add Device |
| 尚未添加任何设备 | No devices added yet |
| 请选择需要连接的设备 | Select the device to connect. |
| 未找到您的设备？ | Can’t find your device? |
| 重新扫描 | Scan Again |
| 连接 | Connect |
| 未搜索到设备 | No device found |
| 请检查设备是否处于可发现状态 | Make sure the device is discoverable. |
| 请将手机尽量靠近要添加的设备 | Keep your phone close to the device you want to add. |
| 重新搜索 | Search Again |
| 连接中，请稍候... | Connecting, please wait... |
| 连接失败 | Connection failed |
| 重新连接 | Reconnect |
| 连接成功 | Connected successfully |

## 页面：Wi-Fi 设置 / 选择 Wi-Fi / 连接状态（对应设计图 22-27、37-38）

| 中文 | 英文建议 |
|---|---|
| Wi-Fi设置 | Wi-Fi Settings |
| Wi-Fi 设置 | Wi-Fi Settings |
| 请选择Wi-Fi | Select Wi-Fi |
| 选择Wi-Fi | Select Wi-Fi |
| 选择 Wi-Fi | Select Wi-Fi |
| 更换Wi-Fi | Change Wi-Fi |
| 请输入Wi-Fi密码 | Wi-Fi password |
| 取消 | Cancel |
| 确定 | Confirm |
| 连接失败 | Connection failed |
| 设备无响应，请将设备靠近路由器重试 | Device not responding. Move the device closer to the router and try again. |
| Wi-Fi已连接成功 | Wi-Fi connected successfully |
| 已连接 | Connected |
| 修改Wi-Fi | Change Wi-Fi |

## 页面：CGM Connection / NS 绑定（对应设计图 28-30、39）

| 中文 | 英文建议 |
|---|---|
| 血糖源设置 | CGM Connection |
| 输入手机号以查询NS账号 | Mobile number for NS account lookup. |
| 请输入手机号 | Mobile number |
| 请输入验证码 | Verification code |
| 获取验证码 | Get Code |
| 查询 | Search |
| 23 秒后重发 | Resend in 23s |
| 绑定 | Bind |
| NS绑定成功 | NS bound successfully |
| 已绑定NS | NS Bound |
| NS网址 | NS URL |
| 修改血糖源 | Change CGM Connection |

## 页面：设备详情 / 我的设备 / 解绑设备（对应设计图 31-36、54）

| 中文 | 英文建议 |
|---|---|
| 我的设备 | My Device |
| 糖哨兵 | GluSentry |
| 设备已离线 | Device offline |
| 添加其他设备 | Add Another Device |
| Wi-Fi设置 | Wi-Fi Settings |
| 血糖源设置 | CGM Connection |
| 模式设置与预览 | Mode Settings & Preview |
| 解绑设备 | Unbind Device |
| 确定解绑设备吗？ | Unbind this device? |
| 蓝牙已断开，尝试连接中... | Bluetooth disconnected. Reconnecting... |
| 蓝牙未打开 | Bluetooth is off |
| 请确保设备已开机并在蓝牙范围内 | Make sure the device is powered on and within Bluetooth range. |
| 连接成功 | Connected successfully |
| 蓝牙未打开 | Bluetooth is off |
| 连接失败 | Connection failed |

## 页面：Units（新增绑定阶段与“我的”修改入口）

| 中文 | 英文建议 |
|---|---|
| 血糖单位 | Units |
| 选择血糖单位 | Select Units |
| 设备和 App 将按所选单位显示血糖值。 | The device and app will display readings in the selected unit. |
| 选择后仍可在“我的”中修改。 | You can change this later in Me. |

## 页面：模式设置与预览 / 多种模式（对应设计图 44-53）

| 中文 | 英文建议 |
|---|---|
| 模式设置与预览 | Mode Settings & Preview |
| 工作模式 | Work Mode |
| 居家模式 | Home Mode |
| 展示预览 | Display Preview |
| 正常状态 | In Range |
| 高、低血糖状态 | High / Low Glucose |
| 无数据状态 | No Data |
| 设备离线状态 | Device Offline |
| 设备已离线 | Device offline |
| 请检查您的血糖数据源是否正常同步数据 | Check whether your glucose data source is syncing properly. |
| 显示与声音 | Display & Sound |
| 屏幕亮度 | Screen Brightness |
| 设备音量 | Device Volume |
| 警报设置 | Alert Settings |
| 高血糖 | High Glucose |
| 低血糖 | Low Glucose |
| 无数据 | No Data |
| 设备离线 | Device Offline |
| 报警方式 | Alarm Methods |
| 声音警报 | Sound Alarm |
| 屏幕闪烁警报 | Screen Flash Alarm |
| 振动警报 | Vibration Alarm |
| 设备发光 | Device Glow |
| 开启后，设备平常发暖黄色光，报警时发橙红色光。 | When enabled, the device glows warm yellow normally and orange-red during alarms. |
| 警报音模式 | Alarm Sound Mode |
| 规律 | Regular |
| 随机 | Random |
| 小睡功能 | Snooze |
| 小睡时长 | Snooze Duration |
| 开启后，短按功能键可暂停报警；小睡结束后，若仍超阈值将恢复报警。 | When enabled, press the device function button to pause alarms. If the alarm condition is still active after the snooze duration ends, alarms will resume. |
| 应用 | Apply |
| 已应用 | Applied |
| 温馨提示 | Notice |
| 您已修改了警报设置，但尚未保存，这些修改将不会生效，是否继续？ | You have changed the alert settings but have not saved them. These changes will not take effect. Continue? |
| 保存失败，请检查网络连接 | Save failed. Check your network connection. |

## 页面：我的与账号（新增“我的”页）

| 中文 | 英文建议 |
|---|---|
| 我的 | Me |
| 基础信息 | Basic Information |
| 账号与安全 | Account & Security |
| 多语言 | Language |
| 我的设备 | My Device |
| 退出登录 | Sign Out |
| 密码设置 | Password Settings |

## 页面：基础信息（新增，供糖宝参考）

| 中文 | 英文建议 |
|---|---|
| 基础信息 | Basic Information |
| 让糖宝更懂您 | Help Tangbao Understand You Better |
| 完善基础信息后，糖宝可结合您的血糖与健康资料，提供更个体化的分析与建议。 | Complete your basic information so Tangbao can provide more personalised insights and suggestions based on your glucose and health profile. |
| 希望糖宝怎么称呼您 | What would you like Tangbao to call you? |
| 性别 | Sex |
| 男 | Male |
| 女 | Female |
| 出生日期 | Date of Birth |
| 身高 | Height |
| 体重 | Weight |
| 患病类型 | Diabetes Type |
| 确诊时间 | Diagnosis Date |
| 治疗方案 | Treatment Plan |
| 请输入 | Input |
| 请选择 | Select |
| T1DM（1型糖尿病） | T1DM (Type 1 Diabetes) |
| T2DM（2型糖尿病） | T2DM (Type 2 Diabetes) |
| GDM（妊娠糖尿病） | GDM (Gestational Diabetes) |
| 其他类型糖尿病 | Other Diabetes Type |
| 胰岛素泵 | Insulin Pump |
| 餐前+基础 | Bolus + Basal |
| 预混 | Premixed Insulin |
| 降糖药物治疗 | Glucose-Lowering Medication |
| 生活方式干预 | Lifestyle Intervention |
| 周制剂 | Weekly Injectable |
| 混合闭环胰岛素输注系统 | Hybrid Closed-Loop Insulin Delivery System |
| 胰岛素类型 | Insulin Type |
| 胰岛素泵类型 | Insulin Pump Type |
| 血糖监测 | Glucose Monitoring |
| 这些信息仅用于糖宝后续交流时参考，不会用于其他用途，也不会向任何第三方共享，请放心填写。 | This information will only be used by Tangbao as context for future conversations. It will not be used for any other purpose or shared with third parties. |

## 页面：多语言（新增）

| 中文 | 英文建议 |
|---|---|
| 多语言 | Language |
| 简体中文 | Simplified Chinese |
| English | English |
| 保存失败，请重试 | Couldn’t save. Please try again. |

## 页面：弹窗与异常提示（新增功能弹窗 + 既有异常）

| 中文 | 英文建议 |
|---|---|
| 确认关闭全部报警方式？ | Turn off all alarm methods? |
| 关闭后，设备可能无法在警报条件触发时提醒你。建议至少保留一种报警方式。 | If all alarm methods are turned off, the device may not be able to alert you when an alarm condition is triggered. We recommend keeping at least one alarm method enabled. |
| 保留报警 | Keep Alarm On |
| 仍要关闭 | Turn Off Anyway |
| 保存失败，请重试 | Couldn’t save. Please try again. |
